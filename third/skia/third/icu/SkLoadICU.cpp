...
static bool load_from(const std::wstring& dir) {
    ...
    fprintf(stderr, "[SkIcuLoader] Trying to load: %ls\n", sPath.c_str());
    if (void* addr = win_mmap(sPath.c_str())) {
        fprintf(stderr, "[SkIcuLoader] win_mmap succeeded for: %ls\n", sPath.c_str());
        if (init_icu(addr)) {
            fprintf(stderr, "[SkIcuLoader] init_icu succeeded for: %ls\n", sPath.c_str());
            return true;
        } else {
            fprintf(stderr, "[SkIcuLoader] init_icu FAILED for: %ls\n", sPath.c_str());
        }
    } else {
        fprintf(stderr, "[SkIcuLoader] win_mmap FAILED for: %ls\n", sPath.c_str());
    }
    return false;
}
...
bool SkLoadICU() {
    ...
    std::call_once(flag, []() {
        auto exeDir = executable_directory();
        auto libDir = library_directory();
        fprintf(stderr, "[SkIcuLoader] executable_directory: %ls\n", exeDir.c_str());
        fprintf(stderr, "[SkIcuLoader] library_directory:   %ls\n", libDir.c_str());
        good = load_from(exeDir) || load_from(libDir);
        fprintf(stderr, "[SkIcuLoader] SkLoadICU result: %s\n", good ? "SUCCESS" : "FAILED");
    });
    return good;
}
