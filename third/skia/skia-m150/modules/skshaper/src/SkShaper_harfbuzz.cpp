...
#if defined(SK_UNICODE_ICU_IMPLEMENTATION)
    fprintf(stderr, "[get_unicode] SK_UNICODE_ICU_IMPLEMENTATION defined, calling SkUnicodes::ICU::Make()\n");
    if (auto unicode = SkUnicodes::ICU::Make()) {
        fprintf(stderr, "[get_unicode] SkUnicodes::ICU::Make() succeeded\n");
        return unicode;
    }
    fprintf(stderr, "[get_unicode] SkUnicodes::ICU::Make() returned null!\n");
...
#endif
    fprintf(stderr, "[get_unicode] All unicode backends failed, returning null\n");
    return nullptr;
}
...
std::unique_ptr<SkShaper> ShapeThenWrap(sk_sp<SkUnicode> unicode,
                                        sk_sp<SkFontMgr> fallback) {
    fprintf(stderr, "[ShapeThenWrap] unicode=%s, fallback=%s\n",
            unicode ? "non-null" : "null", fallback ? "non-null" : "null");
    if (!unicode) {
        fprintf(stderr, "[ShapeThenWrap] unicode is null, returning nullptr\n");
        return nullptr;
    }
    HBBuffer buffer(hb_buffer_create());
    if (!buffer) {
