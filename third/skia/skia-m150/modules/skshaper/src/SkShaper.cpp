...
std::unique_ptr<SkShaper> SkShaper::Make(sk_sp<SkFontMgr> fallback) {
    ...
    fprintf(stderr, "[SkShaper::Make] SK_SHAPER_HARFBUZZ_AVAILABLE && SK_SHAPER_UNICODE_AVAILABLE defined, trying MakeShapeThenWrap\n");
    std::unique_ptr<SkShaper> shaper = MakeShapeThenWrap(std::move(fallback));
    if (shaper) {
        fprintf(stderr, "[SkShaper::Make] MakeShapeThenWrap succeeded\n");
        return shaper;
    }
    fprintf(stderr, "[SkShaper::Make] MakeShapeThenWrap returned null, falling back to Primitive\n");
#elif defined(SK_SHAPER_CORETEXT_AVAILABLE)
    if (auto shaper = SkShapers::CT::CoreText()) {
        return shaper;
    }
#else
    fprintf(stderr, "[SkShaper::Make] No HarfBuzz/CoreText available, using Primitive\n");
#endif
...
public:
    ...
    void consume() override {
        ...
        // If not, try to find a fallback typeface
            ...
            fprintf(stderr, "[FontMgrRunIterator] U+%04X not in primary font, calling matchFamilyStyleCharacter...\n", (unsigned)u);
            sk_sp<SkTypeface> candidate(fFallbackMgr->matchFamilyStyleCharacter(
                fRequestName, fRequestStyle, &language, languageCount, u));
            if (candidate) {
                fprintf(stderr, "[FontMgrRunIterator] U+%04X fallback found: %p\n", (unsigned)u, candidate.get());
                fFallbackFont.setTypeface(std::move(candidate));
                fCurrentFont = &fFallbackFont;
            } else {
                fprintf(stderr, "[FontMgrRunIterator] U+%04X NO fallback found! Using primary font (tofu).\n", (unsigned)u);
                fCurrentFont = &fFont;
            }
        }
        ...
        while (fCurrent < fEnd) {
            ...
            if (!fCurrentFont->unicharToGlyph(u)) {
                const char* language = fLanguage ? fLanguage->currentLanguage() : nullptr;
                int languageCount = fLanguage ? 1 : 0;
                fprintf(stderr, "[FontMgrRunIterator] U+%04X not in current font, calling matchFamilyStyleCharacter...\n", (unsigned)u);
                sk_sp<SkTypeface> candidate(fFallbackMgr->matchFamilyStyleCharacter(
                    fRequestName, fRequestStyle, &language, languageCount, u));
                if (candidate) {
                    fprintf(stderr, "[FontMgrRunIterator] U+%04X fallback found: %p\n", (unsigned)u, candidate.get());
                    ...
                    fprintf(stderr, "[FontMgrRunIterator] U+%04X NO fallback found!\n", (unsigned)u);
                }
            }
        }
    }
