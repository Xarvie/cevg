...
#if defined(SK_USING_THIRD_PARTY_ICU) && defined(SK_BUILD_FOR_WIN)
    fprintf(stderr, "[SkUnicode_icu] Calling SkLoadICU()...\n");
    bool icuLoaded = SkLoadICU();
    fprintf(stderr, "[SkUnicode_icu] SkLoadICU() returned: %s\n", icuLoaded ? "true" : "false");
    if (!icuLoaded) {
        fprintf(stderr, "[SkUnicode_icu] SkLoadICU() FAILED! Returning nullptr.\n");
        static SkOnce once;
        once([] { SkDEBUGF("SkLoadICU() failed!\n"); });
        return nullptr;
    }
#endif
    auto* icuLib = SkGetICULib();
    fprintf(stderr, "[SkUnicode_icu] SkGetICULib() returned: %s\n", icuLib ? "non-null" : "null");
    if (icuLib) {
        fprintf(stderr, "[SkUnicode_icu] Creating SkUnicode_icu instance.\n");
        return sk_make_sp<SkUnicode_icu>();
    }
    fprintf(stderr, "[SkUnicode_icu] SkGetICULib() is null! Returning nullptr.\n");
    return nullptr;
}
}  // namespace SkUnicodes::ICU
